/* ============================================================
   K.RIDEUS — Main JavaScript (tmp/rebuild-v2.1 전용)
   src/js/main.js 사본에서 NAV 3섹션(navbar scroll / mobile menu /
   language selector)을 제거한 버전.

   이유: rebuild-v2.1은 새 navbar 구조(#nav·#navBurger·#navDrawer·
   #navLang)를 쓰고, nav 배경전환·모바일 drawer·언어토글은 전부
   tmp/rebuild-v2.1/js/includes.js가 자체 처리한다. src main.js는
   구(舊) id(#navbar·#menuToggle·#langBtn)를 가드 없이 잡아
   `null.classList` 에러로 IIFE 전체가 중단 → 본문 .reveal 27개가
   opacity:0으로 멈추는 문제가 있었다. 그 충돌 섹션만 들어냈다.

   유지: smooth scroll / hero canvas / scroll reveal / brand video
   (YouTube IFrame) / mobile-br / hero slideshow / service tabs.
   ============================================================ */
...
(function () {
  ...
  /* NAVBAR scroll / MOBILE MENU / LANGUAGE SELECTOR 섹션은 제거됨.
     → tmp includes.js의 .nav.is-solid 토글 / #navBurger drawer /
       [data-setlang] 언어토글이 담당. */
  ...
  function initPlayer() {
    if (!document.getElementById('brandVideoPlayer')) return;
  ...
  // brand video 요소가 있을 때만 YouTube API 로드
  if (videoSection || document.getElementById('brandVideoPlayer')) {
    loadYouTubeAPI();
  }
